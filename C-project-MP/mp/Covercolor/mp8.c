...
basicFlood (int32_t width, int32_t height,
	    ...
	    uint8_t* outBlue)
{ 	//inithial the mark array
	memset(outRed,0,width*height-1);
	//call the recurse function once
	basicRecurse(width, height, inRed, inGreen, inBlue, startX, startY, outRed);
	//overflow the pixel with outRed == 1 and put the input in output when outRed == 0.
